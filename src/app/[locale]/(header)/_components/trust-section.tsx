...
export function TrustSection() {
    ...
    return (
        <section className="w-full bg-gradient-to-b from-sky-50 via-white to-white py-20">
            <div className="mx-auto max-w-6xl px-6">
                ...
                <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    {trustItems.map((item) => (
                        <div
                            key={item.titleKey}
                            className="rounded-3xl border border-sky-100 bg-white/90 p-6 shadow-[0_10px_30px_rgba(14,165,233,0.08)] transition hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-[0_14px_40px_rgba(14,165,233,0.14)]"
                        >
                            <div className="inline-flex rounded-full bg-sky-50 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-sky-700 ring-1 ring-sky-100">
                                {t(item.badgeKey)}
                            </div>
                ...
                <div className="mt-14 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
                    <div className="rounded-3xl border border-sky-200 bg-gradient-to-br from-sky-600 via-blue-600 to-indigo-600 p-8 text-white shadow-[0_18px_50px_rgba(37,99,235,0.22)]">
                    ...
                    <div className="rounded-3xl border border-sky-100 bg-white p-6 shadow-[0_10px_30px_rgba(14,165,233,0.08)]">
                            ...
                            {featuredCompanies.map((company) => (
                                ...
                                >
                                    <div className="flex items-start justify-between gap-4">
                                        ...
                                        <div className="rounded-full bg-white px-3 py-1 text-xs font-medium text-sky-700 ring-1 ring-sky-100">
                                            {t(company.cityKey)}
                                        </div>
                                    </div>
                                    <div className="mt-3 text-xs font-medium uppercase tracking-wide text-sky-700/80">
                                        {t(company.metaKey)}
                                    </div>
                                </div>
                            ))}
                        ...
                        <Link href="/map" prefetch>
                            <button className="mt-6 inline-flex rounded-2xl bg-sky-600 px-5 py-3 text-sm font-medium text-white transition hover:bg-sky-700">
                                {t('viewAll')}
                            </button>
                        </Link>
                    </div>
