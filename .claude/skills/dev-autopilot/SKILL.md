...
- Uruchom pelna walidacje przed commitem (npx tsc --noEmit, npx jest, npx next lint; npm run build tylko gdy faza dotyka routingu/next.config.ts)
...
│           wspomina o XSS, N+1, autoryzacji, typowanie, wzorce, SOLID itp.
│
│  AKCJA:
...
└─ Typ C: WERYFIKACJA E2E (Weryfikacja: / oznaczenie 🌐)
   ...
   2. Napraw przyczyne
   3. Re-uruchom weryfikacje w przegladarce PRZEZ chrome-devtools MCP:
      - Sprawdz czy dev server odpowiada (curl localhost:3000 -> 200/307) i czy baza zyje (docker ps)
      - Nawiguj na URL scenariusza (pamietaj o locale: pl bez prefiksu, en/ru/uk z prefiksem)
      - Zrob snapshot drzewa dostepnosci, wykonaj interakcje, zweryfikuj oczekiwana role + nazwe
      - Sprawdz logi konsoli (blad JS = fail, nawet gdy UI wyglada poprawnie)
      - Screenshot przy failu (zapisz do $1/)
   4. Odznacz checkbox Weryfikacja: DOPIERO po PASS scenariusza.
...
1. Uruchom pelna walidacje (npx tsc --noEmit, npx jest, npx next lint, npm run build)
...
1. Jesli istnieje `package.json`:
   ...
   - W tym repo NIE MA skryptow `typecheck` ani `test`: uzyj `npx tsc --noEmit` i `npx jest`. Lint: `npx next lint`.
...
4. Build: `npm run build` (ostatni krok — najciezszy, ale jako jedyny lapie zlamanie granicy server/client)
...
| E2E Agent 5 zwraca FAIL | Traktuj jako P2. Fix agent musi re-uruchomic scenariusz w przegladarce po naprawie kodu i zweryfikowac snapshot przed odznaczeniem checkboxa. |
