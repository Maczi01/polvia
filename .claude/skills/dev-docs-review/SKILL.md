...
- Sprawdź czy IU miało wypełnione `Delegate to:`. Brak pola w IU sprzed reformy delegacji = ⚪ [info] (legacy plan, nie blokuje review). Niezgodność `Delegate to:` z faktyczną kategorią plików (np. UI files w IU oznaczonym `feature-builder-web-data`) = 🟡 [P3-nit] z notatką dla planisty
...
Skup się na: autoryzacja w Server Actions i route handlerach, SQL injection w Drizzle, XSS (w tym MDX), data exposure, walidacja Zod na granicy, rate limiting, sekrety poza `env.ts`, wycieki przez `NEXT_PUBLIC_*`.
...
**Agent 5: E2E Web Verification (chrome-devtools MCP)**
```
Jesteś testerem E2E weba. Przeczytaj `.claude/agents/feature-tester-web-e2e.md` i zastosuj jego metodologię.
Zbierz niezaznaczone checkboxy `Weryfikacja:` z fazy $2 w pliku zadań $1, które dotyczą scenariuszy w przeglądarce (wejście na URL, klik, formularz, walidacja, i18n, stan po mutacji, brak błędów konsoli).
Pomiń checkboxy CLI (npx tsc, npx jest, npx next lint, grep itp.) i Manual operatora (operacje na bazie, ręczne sprawdzenie przez QA) — zostaną obsłużone w kroku 4.7.
Preflight: dev server na :3000 (`curl -s -o /dev/null -w "%{http_code}" localhost:3000` — 200/307) i baza (`docker ps`). Potem weryfikuj przez narzędzia `mcp__chrome-devtools__*` (nawigacja, snapshot drzewa dostępności, interakcje, logi konsoli). Asercje po roli i dostępnej nazwie, nie po klasach CSS.
Zwróć USTRUKTURYZOWANY wynik per scenariusz w formacie:
  - {treść checkboxa} → passed | failed | skipped (z powodem dla skipped, np. "dev server nie odpowiada", "baza nie wstała")
Klasyfikuj findingi: ✅ passed, 🟠 [P2-important] failed, ⚪ skipped (brak środowiska).
```
...
|---|---|---|
| **CLI** | `npm run`, `npx`, `tsc`, `jest`, `next lint`, `eslint`, `drizzle-kit`, `grep` | Uruchom komendę przez Bash. Jeśli exit 0 → odznacz `[x]`. Jeśli != 0 → zostaw `[ ]`, dopisz suffix ` (FAIL: <skrót błędu>)` i dodaj wpis do raportu jako 🟠 [P2-important]. |
...
| **E2E web (chrome-devtools)** | wejście na URL/stronę, klik, wypełnienie formularza, "w snapshocie widoczne", "bez błędów konsoli", weryfikacja na drugim locale, oznaczenie 🌐 | Sprawdź wynik Agent 5 z mapy zachowanej w 4.6. PASS → `[x]`. FAIL → `[ ]` (P2 już zarejestrowany w 4.6). SKIP → `[ ]` z suffixem ` (SKIP — Agent 5 niedostępny: <powód>)` i wpis P2. |
...
| **Niejasne** | nic z powyższych nie pasuje | Zostaw `[ ]`. Dopisz suffix ` — klasyfikacja niejasna, wymaga ręcznej decyzji`. Dodaj do raportu jako 🟡 [P3-nit] z notatką dla planisty: "checkbox nieautomatyzowalny — rozważ przeniesienie do Operator checklist (dev-plan §3.4) lub przeformułowanie na CLI/E2E w przeglądarce". |
...
- Odznaczone na podstawie Agent 5 E2E (chrome-devtools): Y
...
📊 Statystyki:
   ...
   - ☑️ Weryfikacja: X auto / Y E2E web / Z manual / W niejasne / V failed
