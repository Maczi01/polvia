...
- Dla każdego scenariusza `[E2E]` potwierdź, że jeśli wymaga on konkretnych danych, IU ma w `**Pliki:**` wpisany plik seeda (`src/db/seed.ts` lub dedykowany `src/db/seed-<feature>.ts`, idempotentny). Przygotowanie danych NIE może wisieć pod checkboxem `[E2E]`/`Weryfikacja:` (to tylko wykonanie scenariusza przez testera) — inaczej nikt ich nie napisze i E2E cicho spadnie do Operatora. Autopilot aplikuje schema + seed na bazie z `.env.e2e` per faza. Projekt bez `.env.e2e` → scenariusz `[E2E]` trafia do `Operator checklist` jako `[Manual]`. Nigdy nie kieruj E2E na bazę dev.
...
2. **Wygeneruj trzy pliki:**
   ...
   **W `[nazwa-zadania]-kontekst.md` dodaj sekcję "Designerski kontekst"** (przepisana 1:1 z frontmatera planu technicznego, sekcja 1b Fazy 1):
   ...
   > Te pliki są MANDATORY context dla subagentów buildujących UI. `dev-docs-execute` wstrzykuje je do promptu Agent tool. Tester `feature-tester-web-e2e` używa `figma_screens` do visual diff w przeglądarce (viewport = wymiary frame'a).
   ```
