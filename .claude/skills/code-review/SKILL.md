---
name: code-review
description: "Przeprowadza code review dla Next.js 15 (App Router), React 19, Tailwind v3, shadcn/ui, Drizzle/Postgres, next-intl. Używaj przy przeglądaniu PR, ocenie implementacji fazy/etapu, weryfikacji zgodności z planem. Generuje raport z klasyfikacją problemów (krytyczne/poważne/drobne/sugestie)."
---
...
| `*.tsx` z hooks | React 19, granica RSC/Client (zbędne `'use client'`, fetch w useEffect) |
| `src/db/**`, `src/lib/queries.ts`, `drizzle/**` | Drizzle / Postgres (where, kolumny, migracje, N+1) |
| `_actions.ts`, `src/app/api/**/route.ts` | Bezpieczeństwo (Zod, autoryzacja, rate limiting), Data fetching |
| `src/app/**/page.tsx`, `layout.tsx` | Data fetching (RSC), next-intl, Wydajność |
| `messages/*.json`, użycia `useTranslations` | next-intl (klucze w 4 locale, Link z @/i18n/navigation) |
| Komponenty UI, `globals.css` | Tailwind CSS 3, shadcn/ui, Dostępność |
| `env.ts`, nowe integracje | Bezpieczeństwo (sekrety, NEXT_PUBLIC_, server-only) |
...
🔴 [blocking] KRYTYCZNE — blokuje merge
   ...
   - Crash/utrata danych
   - Wycieki danych (np. select bez wypisanych kolumn, pełny rekord w props Client Component)
   - Brak `error.tsx` / obsługi błędu w krytycznej ścieżce
   - Mutacja (Server Action / route handler) bez sprawdzenia uprawnień — nie ma RLS, kod jest jedyną bramką
   - Sekret w `NEXT_PUBLIC_*` albo w bloku `client` w `env.ts`

🟠 [important] POWAŻNE — wymaga poprawy
   - Drizzle: `update`/`delete` bez `where`, brak walidacji Zod na granicy
   - Problemy wydajnościowe (N+1, brak indeksu, brak paginacji)
   - Brak WCAG compliance
   - Niespełnione wymagania
   - Fetch w `useEffect` zamiast pobrania danych w Server Component
   - Brak klucza tłumaczenia w części locale (`messages/*.json`) — runtime error
