...
dedup ani obsługi race conditions. Pobierz dane w Server Component przez @/lib/queries.
...
- Mutacja bez sprawdzenia uprawnień (nie ma RLS — kod jest jedyną bramką)
...
````
````markdown
🔴 [blocking] **src/app/[locale]/(dashboard)/dashboard/_actions.ts:42**
`deleteService` nie sprawdza uprawnień ani właściciela rekordu — Server Action jest publicznym endpointem, więc dowolny klient może usunąć cudzy wpis.
...
Rozważ jedno zapytanie z `innerJoin` albo `inArray` zamiast pętli.
````
````markdown
🟠 [important] **src/hooks/useData.ts:12**
useEffect do fetchowania danych — użyj
Server Component + funkcja z `@/lib/queries.ts`.
````
...
FYI: Next cache'uje wynik fetchy i segmentów — po mutacji potrzebny `revalidatePath`/`revalidateTag`.
