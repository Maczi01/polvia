...
Sprawdź czy projekt to **Next.js (web)** czy inny stack. Heurystyka — projekt jest tym stackiem gdy spełniony jest co najmniej jeden warunek:
- Istnieje `next.config.ts` / `next.config.js`
- `package.json` zawiera `"next": "..."` w `dependencies`
- Istnieje folder `src/app/` z plikami `page.tsx` / `route.ts` (App Router)

Jeśli projekt to Next.js (ten przypadek):
- W decyzjach `Delegate to:` używaj **web builderów** (`feature-builder-web-ui` | `feature-builder-web-data` | `feature-builder-web-fullstack`)
- W `Skills in play:` używaj `nextjs-stack-guidelines` jako pierwszego skilla
- Scenariusze E2E delegowane do `feature-tester-web-e2e` (chrome-devtools MCP w prawdziwej przeglądarce; Playwright/Cypress nie są zainstalowane — nie planuj plików `*.spec.ts`)
- Server-side code → Server Actions (`_actions.ts`) dla mutacji z UI tego repo, route handlers (`src/app/api/**/route.ts`) dla konsumentów zewnętrznych
- Warstwa danych → Drizzle ORM + Postgres (`src/db/schema.ts`, query w `src/lib/queries.ts` z `server-only`). Nie ma Supabase ani RLS — autoryzacja jest w kodzie
- Zmienne środowiskowe → `env.ts` (`@t3-oss/env-nextjs`), nigdy `process.env` bezpośrednio
- i18n → next-intl, 4 locale (`pl` domyślny bez prefiksu, `en`/`ru`/`uk` z prefiksem), każdy nowy klucz do wszystkich `messages/*.json`
...
- Jeśli **nie istnieje** → zadaj `AskUserQuestion`:
  ...
  2. `Pomiń dla tej iteracji` — kontynuuj bez `DESIGN.md`, zapisz `design_md: null` w frontmatter, dodaj do "Otwarte pytania → Odroczone do implementacji" wpis: "Brak `docs/DESIGN.md` — buildery UI bazują tylko na nextjs-stack-guidelines i SPEC per-feature. Utwórz przed kolejnym UI feature'em."
...
4. Odczytaj `width` i `height` z metadata frame'a (z odpowiedzi `get_design_context`) — to viewport designu dla tego ekranu. **Web uwaga:** najczęstsze wymiary Figma web frame to 1440×1024 i 1280×832 (desktop), 768×1024 (tablet), 390×844 (mobile breakpoint). Tester `feature-tester-web-e2e` użyje tego wymiaru jako viewportu przeglądarki. Zaplanuj, które breakpointy mają mockup — brak mockupu mobile przy responsywnym komponencie to luka w planie, nie w implementacji.
...
- **Delegate to** — subagent wykonujący ten unit (`feature-builder-web-ui` | `feature-builder-web-data` | `feature-builder-web-fullstack`). Reguła decyzyjna w sekcji 3.5.
...
- **Scenariusze testowe** — konkretne zachowania, edge cases i ścieżki awarii do pokrycia. Rozróżniaj typy: `[Unit]` dla testów kodu, `[E2E]` dla scenariuszy do weryfikacji w przeglądarce przez `feature-tester-web-e2e` (chrome-devtools MCP), `[Manual]` dla pojedynczych testów wymagających człowieka (np. weryfikacja na realnym urządzeniu, akceptacja designera). Dla `[E2E]` stosuj konwencję zarządzanego harnessu — patrz sekcja 3.4b.
- **Weryfikacja** — wyłącznie **automatyzowalne** kryteria PASS/FAIL: komenda CLI (`npx tsc --noEmit` / `npx jest` / `npx next lint` / grep) **lub** scenariusz E2E weryfikowalny w przeglądarce przez chrome-devtools MCP. Każdy checkbox `Weryfikacja:` musi być możliwy do domknięcia bez udziału człowieka, wyrażony jako oczekiwany wynik a nie literalny skrypt komend shellowych. Powód: `/dev-docs-review` automatycznie odznacza `Weryfikacja:` po PASS — checkbox nieautomatyzowalny pozostanie wiecznie `[ ]` i zafałszuje raport postępu. Jeśli kryterium wymaga człowieka — przenieś do `Operator checklist` lub do `Scenariusze testowe` jako `[Manual]`
...
Każdy feature-bearing unit powinien zawierać ścieżkę pliku testowego w `**Pliki:**`. Dla unitów modyfikujących strony/komponenty UI lub ścieżki użytkownika — dołącz scenariusze `[E2E]` opisujące flow do przetestowania w przeglądarce (wejście na URL, klik, wypełnienie formularza, asercja po roli i nazwie ze snapshotu, brak błędów konsoli, weryfikacja na drugim locale gdy dotyczy tekstów).

#### 3.4b Zarządzany harness E2E — seedy i baza testowa

Autonomiczne E2E (autopilot) działa na **dedykowanej bazie testowej** opisanej w `.env.e2e` (NIGDY dev/prod). Środowisko stawia i sprząta sam autopilot (dev server Next na `:3000` z env z `.env.e2e`, `npx drizzle-kit push` + `npm run db:seed`); tester `feature-tester-web-e2e` **tylko wykonuje** scenariusze w przeglądarce — nie ma tu plików flow do napisania (chrome-devtools MCP steruje przeglądarką bezpośrednio). Planując scenariusze `[E2E]`, przestrzegaj:

- **Dane testowe wymagane przez scenariusz to deliverable BUILDERA** (kod), nie testera ani Operatora. Jeśli scenariusz potrzebuje konkretnego rekordu (usługa o danym slugu, tag, tłumaczenie), wpisz plik seeda do `**Pliki:**` danego IU jako `Stwórz:`/`Modyfikuj:` (np. `src/db/seed.ts` albo dedykowany `src/db/seed-<feature>.ts`) i przypisz IU do buildera (`feature-builder-web-*`). Checkbox `[E2E]`/`Weryfikacja:` to WYŁĄCZNIE wykonanie scenariusza — przygotowanie danych NIGDY nie może wisieć pod tym checkboxem, bo wtedy nikt ich nie napisze i E2E cicho spadnie do Operatora.
- **Seed musi być idempotentny** (upsert / `onConflictDoNothing`, bezpieczny do re-runu) i nie może zależeć od stałych ID generowanych przez bazę. Wzór: istniejący `src/db/seed.ts`.
- **E2E celuje w bazę z `.env.e2e`** — nigdy nie wstrzykuj danych testowych do bazy dev. Smoke autoryzacji (np. odmowa dostępu do zasobu innego użytkownika) wykonuj przez realne wejście na URL, nie przez SQL.
- **Scenariusz opisuj przez zachowanie, nie przez komendy:** URL wejściowy, akcja, oczekiwana rola + dostępna nazwa w snapshocie, oczekiwany URL końcowy, brak błędów konsoli. Asercje po klasach CSS są zabronione (łamią się przy restylingu).
- **i18n:** jeśli scenariusz dotyczy tekstów, zaplanuj weryfikację na co najmniej dwóch locale (`pl` + jedno z prefiksem) — brakujący klucz w `messages/*.json` objawia się dopiero w runtime.
...
|---|---|---|
| Tylko `*.tsx` w `src/app/**`, `src/components/**`, `src/hooks/**`, `messages/*.json` | `feature-builder-web-ui` | nextjs-stack-guidelines |
| Tylko `*.ts` w `src/db/**`, `src/lib/**`, `src/app/api/**/route.ts`, `_actions.ts`, `drizzle/**`, `env.ts` | `feature-builder-web-data` | nextjs-stack-guidelines, security |
| Mix UI i danych w jednym atomowym IU | `feature-builder-web-fullstack` | nextjs-stack-guidelines, security |

**Reguła praktyczna:** jeśli da się rozsądnie podzielić na dwa osobne IU (jeden UI, drugi data) — podziel. `feature-builder-web-fullstack` używaj **tylko** gdy podział byłby sztuczny (np. formularz z Server Action, gdzie UI bez akcji zapisu lub akcja bez formularza są bezużyteczne).
...
**Design w mirrorze:** `feature-builder-web-ui` i `feature-builder-web-fullstack` konsumują designerski kontekst tylko gdy plan ma niepuste `figma_spec`/`figma_screens` w frontmaterze — wtedy `dev-docs-execute` wstrzykuje subagentowi "Mandatory designerski kontekst" (SPEC.md > DESIGN.md > PNG). Bez tej sekcji w prompcie buildery pracują wyłącznie na wzorcach z repo. `feature-builder-web-data` nigdy nie dostaje kontekstu designu — warstwa danych go nie konsumuje.
...
- Modyfikuj (dane E2E): `src/db/seed.ts` *(idempotentny seed rekordów wymaganych przez scenariusz — patrz 3.4b; pomiń gdy scenariusz nie potrzebuje danych)*

**Delegate to:** feature-builder-web-ui | feature-builder-web-data | feature-builder-web-fullstack
...
- [E2E] [Flow do weryfikacji w przeglądarce: wejście na /mapa, klik filtra X, w snapshocie widoczne Y, zero błędów konsoli]
- [Manual] [Krok wymagający człowieka, np. akceptacja wizualna przez designera] *(opcjonalne — używaj gdy automatyzacja jest niemożliwa)*

**Weryfikacja:** *(wyłącznie automatyzowalne — CLI lub E2E w przeglądarce; rzeczy ręczne idą do Operator checklist niżej)*
- [Komenda CLI z oczekiwanym wynikiem, np. "npx tsc --noEmit przechodzi bez błędów"]
- [Scenariusz E2E z oczekiwanym stanem w snapshocie strony]

**Operator checklist:** *(opcjonalne — kroki wymagające człowieka, NIE odznaczane przez autopilot)*
- [ ] [Krok wymagający operatora, np. "npm run drizzle:push na bazie dev", "QA weryfikuje wygląd na realnym Safari iOS"]
...
- Każdy IU delegowany do `feature-builder-web-ui` lub `feature-builder-web-fullstack` ma w `Skills in play:` `nextjs-stack-guidelines` (mirror per sekcja 3.5)
...
- Każdy checkbox `Weryfikacja:` jest automatyzowalny (CLI lub E2E w przeglądarce przez chrome-devtools MCP). Kroki wymagające człowieka (operacje na bazie, QA, designer) są w `Operator checklist` lub jako `[Manual]` w `Scenariusze testowe` — nigdy w `Weryfikacja:`
