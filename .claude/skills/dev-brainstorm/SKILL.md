...
**Kontekst stacku:** ten projekt to aplikacja **webowa** (Next.js 15 App Router + React 19 + Drizzle/Postgres + next-intl, hosting Vercel). Pytania o SEO i metadata, SSR/RSC vs Client Components, Core Web Vitals, cache i rewalidację, i18n (4 locale), dostępność WCAG, rate limiting są **w scope**. Pytania o store distribution (App Store / Google Play), OTA updates, natywne moduły, biometrię, push notifications są **poza scope** — to nie jest aplikacja mobilna.
