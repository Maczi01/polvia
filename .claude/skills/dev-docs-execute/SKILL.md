...
- `subagent_type` = wartość pola `Delegate to:` z IU (`feature-builder-web-ui` | `feature-builder-web-data` | `feature-builder-web-fullstack`)
...
**Krok 3a — Mandatory designerski kontekst (gdy subagent to `feature-builder-web-ui` lub `feature-builder-web-fullstack`):**
...
- DESIGN.md (projekt-wide tokeny): <ścieżka z design_md, lub "brak — bazuj na nextjs-stack-guidelines">
...
Te pliki są źródłem prawdy o designie. SPEC.md > DESIGN.md > nextjs-stack-guidelines (od najbardziej konkretnego do najbardziej ogólnego). Jeśli SPEC.md nie pokrywa pomiaru — dopytaj Figmę przez MCP Figmy, jeśli jest podłączony (fileKey/nodeId z nagłówka SPEC.md). Nigdy nie zgaduj wymiarów. Web: pomiary z SPEC są w px i przekładają się 1:1 na klasy Tailwind (`p-[18px]`); pamiętaj o breakpointach — mockup desktop nie definiuje zachowania mobile.
```

Jeśli sekcja "Designerski kontekst" nie istnieje LUB wszystkie pola są null/puste → pomiń krok 3a (feature pure-data lub świadoma decyzja "bez Figmy"). Dla subagenta `feature-builder-web-data` zawsze pomijaj krok 3a (warstwa danych nie konsumuje designu).
...
- Checkboxy z prefixem `Weryfikacja:` NIE wykonuj — zostaną zweryfikowane w przeglądarce przez `feature-tester-web-e2e` (chrome-devtools MCP) podczas `/dev-docs-review`
