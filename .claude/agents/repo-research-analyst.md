...
user: "Scope: technology, architecture, patterns. We are building a new notification system with Server Actions and Drizzle."
...
| `next.config.ts` | Next.js configuration and build |
...
- **API surface:** If 0.1 found no web framework or server dependency, **and** the root listing shows no API-related directories or files (`src/app/api/`, `routes/`, `api/`, `_actions.ts`): skip the API surface category. Report "None detected."
- **Data layer:** Evaluate independently from API surface -- a CLI or worker can have a database without any HTTP layer. Skip only if 0.1 found no database-related dependency (e.g., drizzle-orm, prisma, knex, pg) **and** the root listing shows no data-related directories (`drizzle/`, `prisma/`, `migrations/`, `src/db/`). Otherwise, check the data layer table below.
...
| File / Pattern | What it reveals |
|----------------|-----------------|
| `src/app/api/**/route.ts` | Route handlers (HTTP endpoints) |
...
| Schema files (`src/db/schema.ts`, `drizzle/*.sql`) | Data model definitions |
| `drizzle.config.ts` | Drizzle Kit configuration |
| `_actions.ts` (`'use server'`) | Server Actions — mutation + authorization patterns |
...
Common Next.js App Router structures to look for:
...
3. **Documentation and Guidelines Review**
   ...
   - Note testing requirements (Jest + Testing Library + jest-axe, Storybook) and review processes
...
5. **Codebase Pattern Search**
   ...
   - Identify common implementation patterns (Server vs Client Components, query functions, Server Actions, next-intl usage)
...
- Deployment model (monolith, multi-service, serverless, etc.)
- API styles in use (route handlers, Server Actions, REST, etc.)
- Data stores and async patterns
- Module organization style
