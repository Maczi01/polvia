export const meta = {
  ...
  description: 'Wykonanie JEDNEJ fazy zadania: planner czyta IU z docs/plans/, buildery (feature-builder-web-*) implementuja je przez agentType, potem walidacja + commit + aktualizacja dokumentacji.',
    ...
    iu: {
      ...
      items: {
        ...
        properties: {
          ...
          agentType: {
            type: 'string',
            enum: ['feature-builder-web-ui', 'feature-builder-web-data', 'feature-builder-web-fullstack'],
          },
...
4. Wybierz strategie: serial (IU zalezne / wspolne pliki) lub parallel (IU niezalezne).
   Jesli ktorykolwiek IU dodaje nowa zaleznosc (biblioteka, config next/jest) — preferuj serial:
   rownolegle zimne biegi jest po inwalidacji cache duplikuja dlugie transformy i ryzykuja watchdog-kill.
5. Dla kazdego IU zbuduj KOMPLETNY prompt builderowi:
   ...
   - dla feature-builder-web-ui|fullstack: doklej "Mandatory designerski kontekst" z sekcji "Designerski kontekst"
   ...
   Jesli dodajesz zaleznosc (npm install) — odnotuj to w odchyleniach."
...
1. System-Wide Test Check (.claude/skills/dev-docs-execute/SKILL.md sekcja 4.5): typecheck bez nowych bledow,
   ...
   importy nie lamia modulow, granica server/client nie jest zlamana. Komendy: npx tsc --noEmit,
   npx jest, npx next lint (build tylko gdy faza dotyka routingu/metadanych/next.config.ts).
   UWAGA: jesli ktorykolwiek builder raportowal dodanie zaleznosci — pierwszy bieg jest jest ZIMNY (procedura tla z bloku).
${BLOK_DLUGIE_KOMENDY}
